...
def main(argv):
    ...
    if '--rows' in argv:
        ...
        for r in sorted(rows, key=lambda r: (r['battle'], int(r['hit']), int(r['point']))):
            print('%s %s/%s/%s %s server=%s ours=%s angle=%s dir=%s chordDev=%s v=%s -> %s' % (r['at'], r['battle'][:8], r['hit'], r['point'], r.get('part', ''), r.get('server', ''), r.get('ours', ''), r.get('angle', ''), r.get('dir', ''), r.get('chordDev', ''), r.get('v', '?'), classify(r)))
    return 0
