...
If Smart App Control or another application-control policy refuses the installer (Windows error 4551), use the script form, `BullbaHits-<version>-Install.zip`: unpack it anywhere, close the game and double-click `Install.cmd`. No executable of ours is started — `Install.cmd` hands `install.ps1` to Windows' own signed `powershell.exe`, so application control has no unsigned program of ours to block. It finds `C:\Games\World_of_Tanks_NA` by itself; another path can be given as the first argument or typed when asked. It copies the same files with the same checks as the installer, but makes no entry in Programs and Features (so it cannot be uninstalled from there) and creates no desktop shortcut: open `mods\configs\local.armor_inspector\Viewer.html` in a browser. Every file is listed with its SHA-256 in the package's `manifest.json`, and the run writes a log to `mods\configs\local.armor_inspector\installer\`.

Data and the page live in `<game>/mods/configs/local.armor_inspector/`. To move the history, keep that whole folder. The installer keeps records and settings, replaces any previous build of our mod and removes our own leftovers (the backup folder of earlier installers, files the current build no longer ships) without keeping copies; other mods are not touched. After a mod pack that wipes `mods`, Bullba Hits may need to be installed again.
