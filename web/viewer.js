...
(function () {
  ...
  function Viewer(container) {
    ...
    this.targetYaw=this.yaw;this.targetPitch=this.pitch;this.orbitId=null;this.pendingPan=null;
  ...
  Viewer.prototype.startOrbit=function(){
    ...
    var step=function(time){
      ...
      var done=Math.abs(self.targetYaw-self.yaw)<1e-4&&Math.abs(self.targetPitch-self.pitch)<1e-4;
  ...
  Viewer.prototype.reset=function(){this.pan.set(0,0);this.frameCenter.set(0,0);if(this.bounds){this.target.copy(this.pivotCentre());this.grid.position.y=this.bounds.min.y-.025;}if(!this.distanceSet)this.distance=this.defaults.distance;/* default view: the nose towards the viewer and to the right (user, 14.09). The models face -z here: checked by the barrel's extent, not by eye */this.setOrbit(.65+Math.PI/2,.25);this.fitPending=true;this.render();};
  ...
  Viewer.prototype.paint=function(){
    ...
    if(this.heatmap){
      ...
      if(this.surface){try{
        this.surface.hatch=this.dotSpacing;this.surface.dots=this.dots;this.surface.edges=this.partEdges;this.surface.outline=this.zoneOutline;this.surface.tint=this.tint;/* A drag peels at half resolution and without the GPU ricochet trace; the settle redraw and pointerup bring both back. */var size=this.surface.render(this.camera,this.target,this.shell,this.palette,this.trackOpacity,this.dragging?'low':this.quality,this.viewWidth,this.viewHeight,this.renderer.getPixelRatio(),this.dragging?'idle':this.bounceMode);
