(function(){
  ...
  // Laid out the way the garage's own tank-comparison configurator lays it out (user, 20.09): a SLOT
  // first, then the real items of the client grouped by family and grade - not the page's old
  // "kind x variant" grid, which invented four rammers and four "variants" out of the client's own
  // entry prefixes. Every item, every factor and every restriction below comes from
  // window.AIM_CATALOGUE (web/equipment.js), generated by tools/build_equipment_catalogue.py out of the
  // installed client; the page never invents a bonus and never prints the client's internal word
  // "trophy" - the garage calls those pieces Bounty.
  //
  // Sources, in the research report outputs/equipment-perks-2026-09-20.md: devices and their factors
  // in sections 1-2, the slot rule and the slot bonus in section 3, the eligibility filters in
  // section 4, the crew skills in section 6, the directives in section 7 and the consumables in
  // section 8.
  var KMH_TO_MS = 0.27778;   // component_constants.KMH_TO_MS of the client; the record holds m/s
  var CATALOGUE = window.AIM_CATALOGUE || {};
  var AIM_CATS = CATALOGUE.categories || {};
  var AIM_SLOT_TYPES = CATALOGUE.slotTypes || {};
  ...
  var AIM_SLOTS = [0, 1, 2];               // the most optional-device slots any vehicle in the client has
  var AIM_CAT_ORDER = ['firepower', 'mobility', 'survivability', 'stealth'];  // the garage's own order
  ...
  var AIM_GRADE_WORDS = {deluxe: 'of the Improved grade', trophyBasic: 'a Bounty piece',
                         trophyUpgraded: 'an upgraded Bounty piece'};
  function aimNum(v) { return String(Math.round(Number(v) * 10000) / 10000); }
  // --- The client's own rules, read off the catalogue -------------------------------------------
  // THE SLOT BONUS (report section 3). OptionalDevice.defineActiveLevel asks
  // SupplySlotFilter.defineActiveValuesLevel whether the device's categories intersect the SLOT's
  // categories; if they do, LevelsFactor.getActiveValue hands out valueByLevel[1] instead of [0].
  // Only the 42 standard `_tier*` entries carry a category at all, and a real vehicle has at most ONE
  // categorised slot out of three - on 177 vehicles none. The page used to assume the bonus always
  // applied, which made every standard piece look better than it is.
  function aimBonusOn(dev, cat) {
    if (!dev || !cat || !dev.cat || !dev.cat.length) return false;
    for (var i = 0; i < dev.cat.length; i++) if (dev.cat[i] === cat) return true;
    return false;
  }
  // eff is [op, plain] or [op, plain, bonused]; the second figure exists only where the client has two.
  function aimEffValue(eff, bonus) {
    if (!eff) return 1;
    return bonus && eff.length > 2 ? Number(eff[2]) : Number(eff[1]);
  }
  ...
  // What this shooter may mount and what his slots are worth, read off the record the mod wrote.
  // Every field is optional: a battle recorded by an older build carries none of them.
  function aimFitment(vehicle) {
    if (!vehicle) return null;
    var out = {tags: null, level: Number(vehicle.level) > 0 ? Number(vehicle.level) : 0, cats: null};
    var tags = vehicle.tags;
    if (tags && tags.length) {
      out.tags = {};
      for (var i = 0; i < tags.length; i++) out.tags[String(tags[i])] = true;
    }
    var slots = vehicle.supplySlots;
    if (slots && slots.length) {
      var cats = [];
      for (var j = 0; j < slots.length && cats.length < AIM_SLOTS.length; j++) {
        var type = AIM_SLOT_TYPES[String(Number(slots[j]))];
        // Types 6-11 are the consumable, directive, shell and battle-ability slots; only 1-5 hold a device.
        if (type) cats.push(type.length ? String(type[0]) : '');
      }
      out.cats = cats;
    }
    return out;
  ...
  //  slotCat: ['', '', 'firepower'],  the categories those slots grant, from the record or assumed
  ...
  // slotCat is a property of the VEHICLE, not of the build: it never travels in a preset and never
  // takes part in "is this still that preset".
  ...
  // How many optional-device slots this shooter has. The record knows for every vehicle published by a
  // current build; without it the page shows the usual three and says the third one's category is a guess.
  function aimSlotCount() {
    return shooterFit && shooterFit.cats ? shooterFit.cats.length : AIM_SLOTS.length;
  }
  // The categories the slots grant. From <supplySlots> when the record carries it; otherwise two plain
  // slots and one the user picks, defaulting to Firepower - the most common categorised slot after
  // survivability and the one a gunnery build cares about.
  function aimSlotCats(carry) {
    if (shooterFit && shooterFit.cats) {
      var out = shooterFit.cats.slice(0, AIM_SLOTS.length);
      while (out.length < AIM_SLOTS.length) out.push('');
      return out;
    }
    var pick = carry && carry.length === AIM_SLOTS.length ? String(carry[AIM_SLOTS.length - 1]) : 'firepower';
    return ['', '', AIM_CATS[pick] ? pick : ''];
  }
  // The one door every configuration comes through, wherever it came from - a preset, the store, a
  // click or a built-in build. A device the vehicle cannot mount, a device whose archetype another
  // slot already holds and a slot the vehicle does not have are all dropped here and nowhere else.
  function aimValues(base, carry) {
    var out = {slots: ['', '', ''], slotCat: aimSlotCats(carry || (base && base.slotCat)),
               directive: '', food: false, fuel: '', skills: {}};
    if (!base) return out;
    var count = aimSlotCount(), taken = {};
  ...
  function aimPreset(name) {
    ...
    if (built) return aimValues(built.values, shooterConfig && shooterConfig.slotCat);
    return aimStore.presets[name] ? aimValues(aimStore.presets[name], shooterConfig && shooterConfig.slotCat) : null;
  }
  ...
  var AIM_TIER_TAG = {improved: 'deluxe', bounty: 'trophyBasic', bounty_upgraded: 'trophyUpgraded'};
  ...
  // devices in their slots (with the slot bonus only where the categories really intersect), the crew
  // skills and perks that are on, the directive and the consumables.
  function aimEffects() {
    var mul = {}, add = {};
    ...
    AIM_SLOTS.forEach(function (i) {
      var dev = DEVICE_BY_ID[shooterConfig.slots[i]];
      if (!dev) return;
      var bonus = aimBonusOn(dev, shooterConfig.slotCat[i]);
      Object.keys(dev.eff).forEach(function (input) {
        apply(input, [dev.eff[input][0], aimEffValue(dev.eff[input], bonus)]);
      });
    });
  ...
  function syncShooterMods(hit) {
    ...
    shooterType = type;
    // What he may mount and what his slots grant is read before anything is normalised: aimValues
    // drops a device this vehicle cannot take, and it has to know which vehicle that is.
    shooterFit = aimFitment(a);
    var kept = type ? shooterModsState[type] : null;
    if (kept) { shooterConfig = aimValues(kept.values, kept.values.slotCat); shooterPreset = kept.preset || aimPresetMatch(); }
    else {
  ...
  // text label instead of a broken image - which is what most of the new families do today, since only
  // eight of the client's icons have been brought over.
  ...
  // The tooltip of a device: the garage name, the grade, every factor with the input it moves, whether
  // the slot bonus applied here, and the client entry id - so every number on screen can be walked
  // straight back into optional_devices.xml.
  function aimDeviceTitle(dev, cat) {
    var tier = TIER_BY_ID[dev.tier] || {}, bonus = aimBonusOn(dev, cat), parts = [];
    Object.keys(dev.eff).forEach(function (input) {
      var eff = dev.eff[input], v = aimEffValue(eff, bonus);
      parts.push((eff[0] === 'add' ? (v >= 0 ? '+' : '') + aimNum(v) : '×' + aimNum(v))
                 + ' on ' + (AIM_INPUT_WORDS[input] || input));
    });
    var slot;
    if (!dev.cat || !dev.cat.length) slot = 'This grade carries no category, so it never takes a slot bonus.';
    else if (bonus) slot = 'The slot bonus applies: this slot grants ' + (AIM_CATS[cat] || cat)
      + ' and the piece is in that category, so the client hands out the second <valueByLevel> figure.';
    else slot = 'No slot bonus here: this slot grants ' + (cat ? AIM_CATS[cat] || cat : 'no category')
      + ' and the piece is ' + dev.cat.map(function (c) { return AIM_CATS[c] || c; }).join(' / ') + '.';
    return dev.name + ' · ' + (tier.name || dev.tier) + ' · ' + parts.join('; ') + '. ' + slot
  ...
  function aimSlotTile(index) {
    ...
    var cat = shooterConfig.slotCat[index];
    var what = 'Slot ' + (index + 1) + ' · ' + (cat ? AIM_CATS[cat] || cat : 'No Category') + '. ';
    tile.title = dev ? what + aimDeviceTitle(dev, cat) + ' Click to change this slot.'
  ...
  // A tile of the picker: the icon, the grade over the item's own garage name. The tiles carry their
  // text because the client's own icons are shared between the grades of one family - the picture
  // alone cannot tell a Bounty Rammer from a Gun Rammer Class 1.
  function aimPickTile(label, name, title, pressed, disabled, icon, tier) {
    ...
    tile.title = title;
    if (icon) tile.appendChild(aimIcon(icon, aimShort(name)));
    var text = node('span', undefined, 'aim-pick-text');
    text.appendChild(node('span', label, 'aim-pick-tier'));
    text.appendChild(node('span', name, 'aim-pick-name'));
    tile.appendChild(text);
    return tile;
  }
  // The equipment picker, in the shape of the garage's own configurator: one block per family, the
  // family's name and what it does, then one tile per grade the client has for it. Only the pieces
  // THIS vehicle may mount are listed (report section 4); a piece whose archetype another slot
  ...
  function aimPicker(index) {
    ...
    head.appendChild(aimPickTile('Slot ' + (index + 1), 'Empty', 'Leave this slot empty.',
                                 !shooterConfig.slots[index], false, 'empty_slot', 'none'));
    box.appendChild(head);
    AIM_FAMILIES.forEach(function (fam) {
      var offered = AIM_DEVICES.filter(function (d) { return d.family === fam.id && aimFits(d); });
      if (!offered.length) return;
      var caption = node('div', undefined, 'aim-pick-family');
      caption.appendChild(node('b', fam.name));
      caption.appendChild(node('span', ' — ' + fam.what));
      box.appendChild(caption);
      var row = node('div', undefined, 'aim-pick-row');
      offered.forEach(function (dev) {
        var tier = TIER_BY_ID[dev.tier] || {}, clash = aimConflict(dev, index);
        var title = aimDeviceTitle(dev, shooterConfig.slotCat[index])
          + (tier.note ? ' ' + tier.note : '')
          + (clash >= 0 ? ' Already fitted in another slot.' : '');
        var tile = aimPickTile(tier.name || dev.tier, dev.name, title,
                               shooterConfig.slots[index] === dev.id, clash >= 0, dev.icon, dev.tier);
        tile.onclick = function (e) { e.stopPropagation(); aimSetSlot(index, dev.id); };
        row.appendChild(tile);
      });
      box.appendChild(row);
    });
    head.children[0].onclick = function (e) { e.stopPropagation(); aimSetSlot(index, ''); };
    return box;
  }
  ...
  function aimDirectivePicker() {
    var box = node('div', undefined, 'aim-picker');
    var row = node('div', undefined, 'aim-pick-row');
    var empty = aimPickTile('Directive', 'None', 'Leave the directive slot empty.',
                            !shooterConfig.directive, false, 'empty_slot', 'none');
    ...
    AIM_DIRECTIVES.forEach(function (dir) {
      ...
      var tile = aimPickTile(active ? 'Active' : 'Needs its device', dir.name, aimDirectiveTitle(dir),
                             shooterConfig.directive === dir.id, false, dir.icon,
                             active ? 'improved' : 'none');
  ...
  // Consumables and crew are plain toggles: a chip with the name on it, pressed or not. A perk is
  // situational - it holds only while its condition does - and wears a dashed frame and its condition
  // in the tooltip; a skill is always on for a fully trained crew, which is the crew the page models.
  function aimChip(label, title, pressed, onclick, situational) {
    var chip = node('button', label, 'aim-chip');
    chip.type = 'button';
  ...
  function aimConsumableChips() {
    ...
    AIM_CONSUMABLES.forEach(function (c) {
      ...
      row.appendChild(aimChip(c.name, title, on, function () {
  ...
  function aimCrewSection(body) {
    AIM_ROLES.forEach(function (role) {
      ...
      rows.forEach(function (s) {
        chips.appendChild(aimChip(s.name, aimSkillTitle(s), !!shooterConfig.skills[s.id], function () {
  ...
  function buildAimConfig() {
    ...
    // The category of the third slot when the record does not carry <supplySlots>.
    var catRow = node('div', undefined, 'aim-config-row'); catRow.id = 'aim-cfg-cat-row'; catRow.hidden = true;
    catRow.appendChild(node('span', 'Category of slot 3', 'aim-config-note'));
    var cat = node('select'); cat.id = 'aim-cfg-cat';
    cat.title = 'This record does not say which of the vehicle’s slots carries a category, so the page assumes '
      + 'two plain slots and one of this category. A standard device is worth its second <valueByLevel> figure '
      + 'only in a slot whose category it shares; every other grade never gets the bonus at all.';
    var none = node('option', 'No Category'); none.value = ''; cat.appendChild(none);
    AIM_CAT_ORDER.forEach(function (id) {
      if (!AIM_CATS[id]) return;
      var o = node('option', AIM_CATS[id]); o.value = id; cat.appendChild(o);
    });
    cat.onchange = function () { shooterConfig.slotCat[AIM_SLOTS.length - 1] = this.value; aimConfigChanged(); };
    catRow.appendChild(cat);
    aimConfigControls.cat = cat; aimConfigControls.catRow = catRow;
  ...
  function paintAimConfig() {
    ...
    var count = aimSlotCount();
    aimConfigControls.slots.replaceChildren();
    for (var i = 0; i < count; i++) aimConfigControls.slots.appendChild(aimSlotTile(i));
    // Two honest notes instead of a silent assumption: what the record does not know about this vehicle.
    var known = !!(shooterFit && shooterFit.cats);
    aimConfigControls.catRow.hidden = known;
    if (!known) aimConfigControls.cat.value = shooterConfig.slotCat[AIM_SLOTS.length - 1];
    var lines = [];
    if (!count) lines.push('This vehicle has no optional-device slots at all.');
