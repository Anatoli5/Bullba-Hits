...
void main(){
 ...
 outputColor=vec4(color,alpha);
  ...
  function Surface(renderer,engine){
    ...
    // bvhStale: the peel geometry has been posed (pose()) since the GPU BVH was built.
    this.bvhStale=false;
  ...
  // Whether the camera differs from the one the layers were peeled with (cameraCache): the eye, the look direction or
  // the projection (a zoom included). Compared element by element, nothing allocated; the cache is written by
  // keepCamera() when the layers are peeled.
  Surface.prototype.cameraChange=function(camera){
    var cache=this.cameraCache,world=camera.matrixWorld.elements,projection=camera.projectionMatrix.elements,i;
    for(i=0;i<16;i++)if(cache[i]!==world[i]||cache[i+16]!==projection[i])return true;
    return false;
  };
