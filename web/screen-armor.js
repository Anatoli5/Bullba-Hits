...
float evaluate(vec2 uv,out vec4 front,out bool screen,out int screens){
 vec2 first=layer(0,uv);front=vec4(-2.0);screen=false;screens=0;if(first.y<.5)return -3.0;
...
void main(){
 vec4 front;bool screen;int screens;
 float result=evaluate(vUV,front,screen,screens);
 ...
 outputColor=vec4(color,1.0);
  ...
  function Surface(renderer,engine){
    ...
    try{this.update(engine);var error=null,previous=renderer.debug.onShaderError;renderer.debug.onShaderError=function(gl,p,v,f){error=gl.getProgramInfoLog(p)||gl.getShaderInfoLog(f)||'shader error';};try{renderer.compile(this.captureScene,this.captureCamera);var scene=new T.Scene();scene.add(this.quad);renderer.compile(scene,new T.Camera());scene.remove(this.quad);}finally{renderer.debug.onShaderError=previous;}if(error)throw new Error(error);this.quad.visible=false;}catch(e){this.dispose();throw e;}
  }
  ...
  Surface.prototype.dispose=function(){if(this.quad){if(this.quad.parent)this.quad.parent.remove(this.quad);this.quad.geometry.dispose();this.material.dispose();}if(this.mesh)this.mesh.geometry.dispose();if(this.peelMaterial)this.peelMaterial.dispose();if(this.materialTexture)this.materialTexture.dispose();this.targets.forEach(function(t){t.dispose();});if(this.depth)this.depth.dispose();if(this.blank)this.blank.dispose();};
